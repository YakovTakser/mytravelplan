...
from mytravelplan import db, bcrypt, mail
from mytravelplan.main.routes import like_to_post
from mytravelplan.messages.forms import MessageForm
from mytravelplan.messages.routes import messages_counters
from mytravelplan.models import User, Post, Comment, Country, Message, Image
from mytravelplan.posts.forms import LikeForm, ReportForm
from mytravelplan.users.forms import RegistrationForm, LoginForm, UpdateAccountForm, ViewUser, RequestResetForm, \
    ResetPasswordForm
from mytravelplan.messages.forms import MessageClickForm
from mytravelplan.users.utils import save_picture
from flask_mail import Message as MessageMail

users = Blueprint('users', __name__)


# In that file all the routes of the users at the website, here is all the logic and functionality of the users
# and here is the templates that user sees that belong to users
...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data, password=hashed_password,
                    ...
                    date_of_birth=form.date_of_birth.data, admin_permissions=form.admin_permissions.data)
        db.session.add(user)
        db.session.commit()
        send_verification_email(user)
        flash('Your account has been created, Please activate your account via email.', 'success')
        return redirect(url_for('users.login'))
    return render_template('register.html', title='Register', form=form, msg_counters=msg_counters)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user.verified == True:
            ...
            flash('Login Unsuccessful. Please verify your account before log in.', 'warning')
            return render_template('login.html', title='Login', form=form, msg_counters=msg_counters)
...
def send_reset_email(user):
    ...
    msg = MessageMail('My Travel Plan - Password reset Request', sender='noreply@demo.com', recipients=[user.email])
...
def send_verification_email(user):
    ...
    msg = MessageMail('My Travel Plan - Activate your account here', sender='noreply@demo.com', recipients=[user.email])
...
def reset_request():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('users.login'))
    return render_template('reset_request.html', legend='Reset Password', form=form, msg_counters=msg_counters)
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('users.login'))
    return render_template('reset_token.html', legend='Reset Password', form=form, msg_counters=msg_counters)


@users.route('/verification/<token>', methods=['GET', 'POST'])
def verification_token(token):
    ...
    form = LoginForm()
    if user is None:
        flash("That is an invalid or expired token please try to register again.", 'warning')
        return redirect(url_for('users.register'))
    flash("You've activated your account, now you are able to login.", 'success')
    return redirect(url_for('users.login'))
...
def account(username):
    ...
    #                    'America/Argentina/Ushuaia'], 'code': 'AR', 'continent': 'South America',
    ...
    #      'capital': 'Sucre'},
    #     {'timezones': ['America/Noronha', 'America/Belem', 'America/Fortaleza', 'America/Recife',
    #                    'America/Araguaina', 'America/Maceio', 'America/Bahia', 'America/Sao_Paulo',
    #                    'America/Campo_Grande', 'America/Cuiaba', 'America/Porto_Velho', 'America/Boa_Vista',
    #                    'America/Manaus', 'America/Eirunepe', 'America/Rio_Branco'], 'code': 'BR',
    #      'continent': 'South America', 'name': 'Brazil', 'capital': 'Bras\xc3\xadlia'},
    ...
    #                    'Asia/Yakutsk', 'Asia/Vladivostok', 'Asia/Sakhalin', 'Asia/Magadan', 'Asia/Kamchatka',
    #                    'Asia/Anadyr'], 'code': 'RU', 'continent': 'Europe', 'name': 'Russia', 'capital': 'Moscow'},
    ...
    #                    'America/Indiana/Knox', 'America/Indiana/Vevay', 'America/Chicago',
    #                    'America/Indiana/Vincennes', 'America/Indiana/Petersburg', 'America/Menominee',
    #                    'America/North_Dakota/Center', 'America/North_Dakota/New_Salem', 'America/Denver',
    #                    'America/Boise', 'America/Shiprock', 'America/Phoenix', 'America/Los_Angeles',
    #                    'America/Anchorage', 'America/Juneau', 'America/Yakutat', 'America/Nome', 'America/Adak',
    #                    'Pacific/Honolulu'], 'code': 'US', 'continent': 'North America', 'name': 'United States',
    ...
    #      'capital': 'Bangui'},
    #     {'timezones': ['Africa/Ndjamena'], 'code': 'TD', 'continent': 'Africa', 'name': 'Chad',
    #      'capital': "N'Djamena"},
    ...
    #     new_country = Country(country_name=country['name'])
    ...
    if user.username == current_user.username or current_user.admin_permissions == 1:  # Check if wanted user is logged in user or admin
        ...
        if form.validate_on_submit():
            ...
            # Admin permissions
            user.admin_permissions = form.admin_permissions.data
            db.session.commit()
            ...
            # Admin permissions
            form.admin_permissions.data = user.admin_permissions
        ...
        return render_template('account.html', title='Account', image_file=image_file, form=form, user=user,
                               posts=posts, msg_counters=msg_counters)
    else:
        abort(403)


# User account showing to others users
@users.route('/user/<string:username>/', methods=['GET', 'POST'])
def user_account_show(username):
    # Generic functionality
    # Messages counters
    msg_counters = messages_counters()
    message_clicked = MessageClickForm()

    if message_clicked.validate_on_submit():
        if current_user.is_authenticated:
            return redirect(url_for('messages.new_message', receiver=username))
    user = User.query.filter_by(username=username).first_or_404()
    page = request.args.get('page', 1, type=int)
    posts = Post.query.filter_by(user_id=user.id).order_by(Post.date_posted.desc()).paginate(page=page, per_page=5)
    return render_template('user_account_show.html', user=user, form=message_clicked, msg_counters=msg_counters, posts=posts)


# Delete of the user
@users.route("/user_delete/<string:username>/", methods=['POST'])
@login_required
def delete_user(username):
    user = User.query.filter_by(username=username).first_or_404()
    if user != current_user and current_user.admin_permissions != 1:
        abort(403)
    posts = Post.query.filter_by(user_id=user.id)
    comments = Comment.query.filter_by(username=user.username)
...
def admin():
    ...
    if current_user.admin_permissions == 1:
        ...
        form = ViewUser()
        if form.validate_on_submit():  # check if username was inputted
            user = User.query.filter_by(username=form.username.data).first_or_404()
            return redirect(url_for('users.account', username=user.username))
        return render_template('admin.html', title='Admin Panel', form=form, msg_counters=msg_counters)
    else:
        abort(403)


# Reported posts show
@users.route('/reported', methods=['GET', 'POST'])
@login_required
def reported_posts():
    if current_user.admin_permissions == 1:
        # Generic functionality
        ...
        page = request.args.get('page', 1, type=int)
        posts = Post.query.filter_by(reported=True).order_by(Post.date_posted.desc()).paginate(page=page, per_page=5)
        ...
        if reported_form.validate_on_submit():
            ...
            post.reported = True
            db.session.commit()
        return render_template('reported_posts.html', posts=posts, form=form, reported_form=reported_form, msg_counters=msg_counters)
    else:
        abort(403)